...
# Environment variables, set by the main program in train.py.
env = EasyDict()
...
# Global random seed.
random_seed = 1000
# Options for dataset.load_dataset().
dataset = EasyDict()
# Options for main training func.
train = EasyDict(func='train.train_progressive_gan')
# Options for generator network.
G = EasyDict(func='networks.G_paper')
...
G_loss = EasyDict(func='loss.G_wgan_acgan')
# Options for discriminator loss.
D_loss = EasyDict(func='loss.D_wgangp_acgan')
# Options for train.TrainingSchedule.
sched = EasyDict()
...
# Dataset (choose one).
desc += '-customimages'
train.resume_run_id = '000-pgan-customimages-preset-v2-1gpu-fp32'
dataset = EasyDict(tfrecord_dir="/customimages")
...
sched.minibatch_dict = {4: 128, 8: 128, 16: 128,
...
train = EasyDict(func='util_scripts.generate_fake_images',
                 run_id=5, num_pngs=1000)
num_gpus = 1
